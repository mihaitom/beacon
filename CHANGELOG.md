...
- Playlists can now be filled from Last.fm: the charts of a country or the whole world, a genre's or an artist's top tracks, tracks similar to a song of your own picked from the library, or your own most played. Beacon looks each one up in your library and shows you both sides next to each other, numbered by chart position, before anything is saved. The found tracks can also go to the queue with one click instead of being saved as a playlist, either replacing what is playing or adding to it. Naming it after a playlist you already have offers to add only the tracks missing from it, or to replace its contents outright. Needs a Last.fm key, which is entered once in Settings under "Advanced" and then applies to everyone using this Beacon; without one the button is not shown at all
...
- An artist's page now shows the opening paragraph of their Wikipedia article under the name, in the app's language where Wikipedia has one and in English otherwise, with a link to the full article. Up to five lines are shown in full; a longer paragraph is collapsed to three lines behind a "Show more" link
...
- Fanart.tv images now dress the artist surfaces: the artist's background photo across the top of their page and of their albums' pages, fading out towards the content below, and behind Now Playing, plus the artist's clear logo in place of the plain name where Fanart.tv has one. Now Playing shows that background by default, with the album artwork one button away; while the artwork is hidden the track text moves into the corner on a single ellipsised line each, where it stays clear of the lyrics beside it (on a phone the small cover and its labels sit centred along the bottom), and its lyrics now sit on their own translucent panel so they stay readable over any backdrop. Beacon identifies itself to Fanart.tv with its own project key, so this works with no setup, and a switch under Library turns it off; a personal Fanart.tv key is optional and only keeps the images fresher. The images are kept on the server, so they come back instantly instead of being fetched from Fanart.tv again, and where an artist has several, one of the five most-liked is shown at random, with a button in Now Playing's toolbar to step to another of them. A collaboration credit such as "A & B" is dressed with its first performer's images, since the combination has none of its own. Near the end of a track a second card slides in beside the corner showing the next track and its cover (chevrons pointing the way), then takes the current card's place when the change happens, the next artist's images already loaded; a phone has no room for that second card, so there only the images are loaded ahead
...
- Where several people share one Beacon (the Docker and web builds), a server administrator can now control who may cast. The policy is a whitelist or a blacklist, with a separate switch for what accounts not on the list - including ones that appear later - may do; anyone who may not cast can still browse and listen locally. An untouched server lets everyone cast, the administrator is always allowed, and a Plex session does not offer the setting at all. The picker offers the accounts that have signed in to this Beacon, plus Jellyfin's own full list

### Changed

- Settings is grouped into tabs now - Account, Playback, Library, Casting and Advanced - instead of one long scroll, so it stays navigable as more settings are added. Casting appears only where casting can be limited at all, and Advanced (the log level, the API keys, the AirPlay pairings and clearing this device's cache) only for a server administrator on the shared web/Docker build - the single-user desktop app always shows it. The old "show advanced features" switch is gone
- The search in the top bar now runs over the library Beacon already holds instead of asking the music server, so it is forgiving in the same way: accents, punctuation and word order no longer have to match, a slightly misspelled word still finds the song, album or artist, and the closest match is listed first. The Songs and genre track lists order their filtered results the same way, and a third click on a column header returns them to that match order. A switch beside the search field on the library screens, and on the search results page, stops it accepting misspellings when you want to search exactly - a partial word still finds its match. The Radio and Playlists filters are always exact instead, being short name-only lists; on the phone and in the phone remote, which have no room for the switch, misspellings are never accepted
...
- Surround tracks, a 5.1 FLAC for instance, stayed completely silent when cast. They are now folded down to stereo for the speaker, which is what every cast target can actually play, and the stream info names the source's channel count so the downmix has a visible reason. Folding the channels together costs real volume, so a surround track plays noticeably quieter than a stereo one: roughly 8 dB for a five-channel track, and about 10 dB for a 7.1 one. That is the standard behaviour of ffmpeg, the audio engine Beacon uses, rather than a level Beacon sets. Adding five channels together would otherwise overshoot what the audio can hold and distort, so the fold makes room for that first
- The same tracks were silent in this app's own player when the audio quality was set to Opus, and on AAC they played but spread the chosen bitrate across five channels instead of two. Every quality setting now produces a stereo stream, so it sounds as good as the bitrate allows. Playing at "Original" keeps every channel, as it always did
...
- On an iPhone, a song played at a reduced quality setting (AAC, for instance) kept jumping back to where it had started, roughly every 45 seconds. It now plays through at every quality setting, and shows its total time on the lock screen and how far ahead it has loaded on the seek bar, as it does at "Original"
...
- Casting to a Sonos soundbar such as the Beam failed while its TV was on, reported as the speaker refusing the stream, and afterwards playback could not be stopped either. Casting now switches the soundbar over from the TV, and stopping always ends playback in Beacon
- A speaker that cannot be paused, resumed or added to a running cast now gets a proper message in the device list, naming it and what went wrong. Pausing and resuming used to fail without a word, and adding a speaker showed the raw error code. A Sonos speaker that cannot be found on the network is reported as unreachable rather than as a failure
- An AirPlay speaker that refuses to start, cannot be reached or has to be paired first now says so, instead of the app showing the music as playing over silence. A speaker dropping out part-way through a track is offered to resume whatever the cause, where before only one kind of drop was noticed
- A Chromecast that refuses to play a track or station now says so, instead of the app showing it as playing over silence. Pausing or stopping one that has nothing loaded is no longer reported as an error
- Casting to a DLNA renderer no longer risks the next track, a pause or a stop being skipped without a word while the app shows otherwise. A renderer that refuses a stream or cannot be reached now says so
...
- An artist tile now shows a record and a note icon with its two counts, instead of spelling out the album count alone. The track figure counts every track the artist is on, compilations and guest appearances included, which is what makes an artist with no album of their own readable at all where the tile used to say only "0 albums"
...
- A 24-bit AIFF, or a WAV at 32 bit or in floating point, was converted to MP3 on the way to a speaker instead of being sent losslessly
...
- An artist heard only on compilations, or only as a guest on someone else's record, had a completely empty page and played nothing when picked. Their tracks are listed and played now, and a guest appearance turns up on the artist's own page instead of only on the album it came from
- Typing in the search of a station's title history works properly now. A space was deleted again a moment after it was typed, so more than one word was impossible, and typing quickly lost letters every time the results updated
...
- Added a "Discover stations" search to the Radio page, backed by the free Radio Browser directory. Search by name, filter by country (remembered for next time) and sort by most-played or most-voted, then either try a station straight away or add it to your list with one click, instead of typing in a stream URL by hand. Each result shows the station's logo, where it is, what it broadcasts, how popular it is and whether it was reachable at its last check. Beacon reports a listen back to the directory each time you play a station you found there, which is what that popularity ordering is built on
- Right-clicking in the library now offers what that thing can do, not just what fits on the tile: an album can be played, queued or added to a playlist and links through to its artist, an artist offers Artist Radio and their catalogue, a playlist can be renamed or deleted from the overview, and a track gained "Go to album" and "Go to artist"
- Cover art and artist photos can now be looked at full size, filling the window. Click the artwork at the top of any album, artist, genre or playlist page, or right-click a tile or a track and pick "Show image". It is fetched at a higher resolution than the browsing views need. Opening one shows the copy you were already looking at while the full-size version downloads, rather than a placeholder over a picture that was on screen a moment ago.
- Right-clicking a track now offers Info, which opens everything your media server holds about it: the tags, the audio itself (format, bitrate, sample rate, bit depth, channels, file size, path), what the library knows (plays, last played, added, rating, ReplayGain) and its MusicBrainz and ISRC identifiers. Only the fields your own server actually reports are listed. Navidrome answers with the most; the Jellyfin and Plex bridges now pass through everything those two expose for a single track
- The desktop window gained back and forward arrows beside the Beacon name. Until now a page opened from somewhere else was a dead end: open an album from an artist page and the only way back was to search for them again. Alt with the arrow keys does the same, as do a mouse's own back and forward buttons
- Radio stations that broadcast a "now playing" tag show the current artist and track under the station name, in the player bar, on Now Playing, on the phone and the phone remote, and on the lock screen. Not every station sends one, in which case only the station name shows
- While an internet radio station is playing, the lyrics panel shows what the station has played instead, with the time each title came in. Every station keeps its own list, and it survives a restart, since the Beacon server keeps it rather than the app. Clicking a title searches your own library for that track. Things that are not songs (a programme name, a news item) are kept as plain lines rather than guessed at
- Casting an internet radio station now routes through Beacon's own backend by default, so a station is fetched once no matter how many devices are casting it. A new "Send radio straight to the device" setting under Playback (off by default) opts back into the previous behavior
- Local playback now buffers further ahead and rides out a brief loss of connection, such as a dead spot on a mobile network, instead of stopping outright. It quietly retries the same track from where it left off, and the seek bar shows how far ahead it is buffered as a lighter band beyond the played position
- Added account settings that follow the person rather than the device: the language, the personalized-recommendations toggle, the lyrics providers above and whether cast radio goes straight to the device. In the Docker/web version they live with your account on the Beacon server, so every device signed in there picks a change up at its next login; the desktop app keeps them per account on that machine. Settings that are genuinely about one device (audio quality, the saved queue, cached artwork) stay put
- Added a setting for which lyrics providers Beacon may ask. Lyrics stored with the song file itself are always tried first and never leave your server; the three internet providers (lrclib.net, NetEase, SimpMusic) are on by default and can be switched off individually or all at once, if you would rather a song's title, artist, album and duration never be sent anywhere
- Settings has a new Privacy entry listing every outside service Beacon contacts, what each one is asked for and what is sent with the request, split by whether the connection comes from your device or from your Beacon server. Services a setting can switch off say which setting that is
- The phone remote gained the mini player strip above the tab bar that the mobile web view already had: what is playing, with previous, play/pause and next, and a tap anywhere on it opens Now Playing. The mobile web view's own strip gained the previous button
- The Jellyfin Quick Connect code can now be copied with a button beside it, rather than read off the screen and typed out
- With the log level set to Debug or Trace, the fullscreen visualizer now shows what it is working from while casting: the moment it is drawing and the moment playback is actually at, with the difference highlighted once it grows large enough to hear. Invisible at the normal log level

### Changed

- Starting an internet radio station no longer throws the queue away. Whatever was lined up stays where it was and is still in the queue view, and clicking a song in it picks the queue back up at that song
- Playing a track from the search results now plays just that track, instead of putting every other match in the queue behind it. A search is a list of things that contain the same word, not a running order. The same goes for tapping a song in the library on the phone and for Home's most-played list, which is a chart; the play button beside its heading still starts the whole list
- Searching now returns a page's worth of results rather than 25 of each kind. A common first name matches far more than that in any real library, and nothing said the list had been shortened. Songs get the most room, since that is the list you scan
- Search results now show matching artists and albums as scrolling rows with arrows, the same shape Home and your favourites use, instead of a list and a grid that both grew downwards and pushed the matching songs off the screen
- The Songs tab on the phone is now Library, with a Songs/Albums switch above the search box: the same list and search, pointed at whichever you pick. Both the mobile web view and the phone remote get this, and an old "Songs" bookmark still lands in the right place
- Queue, Library, Playlists and Radio on the phone now read as one app. They had drifted into three artwork sizes, three title treatments and four different row heights; artwork, row height, the two lines of text and the line between entries are the same across all four now
- Settings has been reorganised visually: each section is a panel of its own, every setting inside one is separated by a thin line, and explanatory text is consistently quieter than the setting it belongs to. Nothing moved between sections and no setting changed what it does
- Settings that stay on a device are now kept separately per account, so two accounts sharing one computer no longer see each other's audio quality, saved queue or cached library. Whatever is set today carries over to the account that logs in first
- Beacon's copy of your library is no longer kept in a place with a roughly 5 MB ceiling. A large library ran past it and the whole copy was silently dropped, so Beacon re-fetched the entire catalog on every start. That hit Jellyfin hardest, where a full scan takes minutes
- Cover art, artist photos and station logos are now fetched in combined batches, one request per screen instead of one per image, so a large view fills in far fewer round trips
- Lyrics are now remembered by the Beacon server as well as by each device, so a song someone has already looked up costs nothing the second time: on another device, after a reload, or after a restart. That also means far less of your listening leaves your server
- The album grid fills in noticeably faster on a large library: Beacon asks for several pages of albums at once instead of waiting for each before asking for the next
- On Jellyfin, a library Beacon already has is trusted for a day rather than an hour before refreshing in the background. Nothing changes for Navidrome and other Subsonic servers, where that refresh is cheap, and the manual rescan still updates everything immediately
- Beacon now stops its background checks (each speaker's volume, the device list, a station's "now playing" tag) while the window is hidden, and pauses them entirely for a while when a security proxy in front of a self-hosted Beacon starts refusing requests
- While playing an internet radio station, the seek bar is replaced by a small "on air" indicator and how long you have been listening, instead of a progress bar with no position or length to show. It pulses only while sound is actually coming out
- While an internet radio station is playing, the controls that only make sense for a queue (shuffle, repeat, previous, next) are greyed out instead of looking pressable and doing nothing
- The sidebar is now opened and closed with a button at its top, instead of unfolding whenever the mouse passed over it. However you leave it is remembered per device
- The track and artist in the player bar now use the full width available rather than a fixed narrow column, so longer titles are cut off later. It shows most on internet radio, where the station sends both as a single line
- The speaker picker on the phone now reads as one list of places the sound can go, with the current one ticked, instead of two differently coloured rows on top of a list that did the same thing
- Chromecast and DLNA speakers now report their own volume changes instead of being asked every few seconds, the way Sonos already did. Turning the dial on the speaker or in another app shows up in Beacon straight away
- While casting to an AirPlay device, the song position now follows how much of the song has actually been handed over instead of assuming a fixed delay for every track. It stays an approximation, since AirPlay speakers never report where they are, unlike Sonos, Chromecast and DLNA
- The cast panel's "Rescan" button now shows a spinner and stays unavailable until the search has finished, in place of a separate progress bar. The phone's device sheet gained the same button, and the first search after signing in says it is searching rather than "No devices found"
- A radio station's logo is now picked by what the image file actually measures rather than what the station's page claims, and Beacon keeps looking for a larger one when the first is too small for where it will be shown
- Radio station logos are now remembered by the Beacon server across restarts. Finding one means fetching and reading the station's own website, and the desktop app starts its own Beacon with every launch, so until now every launch did that again for every station. They appear immediately, and the stations' sites are left alone for a week at a time
- The Radio page on the phone now lists your stations one per line, the way the phone remote already did, instead of a grid of tiles that only ever fitted one per line anyway. The desktop keeps its tiles
- The page headings across the phone views now match. Queue, Library, Playlists, Radio and a playlist's own page each opened with the same thing spelled four different ways; they share one header row now, and the search box stays in reach while the list scrolls
- On the phone remote, every tab now opens with its own name at the same size, and Radio no longer calls itself "Internet Radio" while its tab says "Radio"
- The two-way switches across the app (ReplayGain, Songs/Albums on the phone, the sort in Discover, Password/Quick Connect for Jellyfin) are now one and the same control, with the selected side lit in Beacon's amber
- The heart for marking a song a favourite is gone from the song lists on the phone. There is no favourites screen there to look at afterwards, so it was an action with no visible result
- Adding a radio station by hand is no longer offered on the phone. It is three URL-shaped text fields, and Discover reaches the same result with a search and one tap. The desktop keeps it
- On Home, clicking the artwork of whatever is playing opens the Now Playing screen, the same shortcut the player bar's artwork already offers
- On the phone remote, an action now waits until Beacon has actually carried it out: the action sheet keeps its spinner until then, and says so if something did not get through. A tap that had not landed used to look exactly like one that had
- The log level under Settings' Advanced section is now only offered to accounts that are also allowed to trigger a library scan
- Various small UI sizing/spacing tweaks: the column headings above every track list read as labels rather than a greyer first row, and the sorted column is lit instead of marked only by a small arrow; the Albums grid spaces its covers like the shelves elsewhere; the playlist and station tiles are bigger; every dialog that can hold a lot opens at the same height and scrolls inside it rather than cutting its last lines off; loading placeholders match the content that replaces them, so nothing shifts as the real covers arrive

### Removed

- The greeting on Home ("Good evening, <name>") is gone. It greeted you by name every time you opened the app, which wears thin fast, and the card now starts straight at what you were listening to
...
- Fixed a cast track sometimes never finishing: it played to its end, but Beacon went on showing it as playing, the position kept climbing past the end and the visualizer stayed frozen on its last moment. Starting the same track twice in quick succession was enough to trigger it, most likely to be noticed with Autoplay off
- Fixed Beacon being able to get a self-hosted instance's own address banned by the security software guarding it. The live-update connection retried every couple of seconds, indefinitely and without slowing down, and the Radio page re-asked for every station logo it had not found, on every single view. The connection now backs off and pauses while the device is offline, and a station with no logo is remembered as having none
- Fixed internet radio station logos, which were missing or wrong in half a dozen unrelated ways. Stations whose website forwards you to another address, or buries the logo under a lot of styling, never had theirs found at all. The phone remote was refused every logo it asked for. A logo could stay blank in the player bar and on Home for days on end while the very same one showed fine on Now Playing, from a browser cache entry that is now stepped around without anything needing to be cleared by hand. And on Now Playing, wide banners were cropped to fill a square and vector logos were framed like square covers instead of floating free. Album art is unaffected by those last two. A station that has been showing no logo can take a few hours to catch up
- Fixed casting to some Chromecast devices, a Google TV among them, staying completely silent with no error shown anywhere. Those devices can keep their own app running instead of switching to the one that plays what is sent to them
- Fixed casting an internet radio station in a format other than MP3, AAC in particular, failing outright with an unsupported-format error on Sonos. Beacon announced every station as MP3 regardless of what it really sends, and now asks the station itself
- Fixed a cast failing outright, or a station staying silent on this device, for radio stations published as a playlist file (a .m3u or .pls address, which names where the audio is rather than being it). Beacon now looks inside one and plays what it points at
- Fixed an internet radio station going silent on a speaker while you were told the wrong thing about why. A station whose address no longer serves refuses every request, which reached you as the speaker reporting access denied
- Fixed picking a speaker another Beacon session is using going wrong in three separate ways: it stopped the speaker you were already listening on and dropped the music back to this device before you had answered the "take it over?" question, and answering it then did nothing
- Fixed moving a cast from one speaker to another. Switching between two Sonos speakers interrupted a third that had nothing to do with the switch, and locked up the play/pause and seek controls for several seconds
- Fixed both device pickers showing a set of speakers that was not real. The phone's sheet applied whatever it showed when it opened, even after the speakers had changed underneath it, and the desktop panel's tick marks briefly flicked back to the previous speaker
- Fixed a speaker that refuses to stop being left listed as still playing with nothing said about it. The failure is now reported and the speaker drops out of the list
- Fixed picking a speaker while playback is paused making a sound. Adding or switching to one while already casting started the music outright, and picking a first one still played a moment of the song before falling silent again
- Fixed artwork staying blank, album covers and radio station logos alike. A first attempt that did not get through was never tried again, and the player bar's very first cover in the installed app was drawn before the connection to your server was ready. Artwork is now retried a few times, and again as soon as the connection is there
- Fixed pressing play doing nothing for an internet radio station that was showing but not yet loaded, right after restarting Beacon with a station selected but paused
...
- Fixed a song that Autoplay had just added to the queue occasionally disappearing again and being replaced by a different one a moment later, when two devices were connected to the same cast session and both topped the queue up at once
- Fixed the volume slider jumping back to where it was a moment after being moved, on a speaker as well as on the phone remote
- Fixed the elapsed time on the seek bar jumping back a second or two every so often while casting, which pulled the synced lyrics back a line with it each time. Having the speaker picker open was enough to cause it on its own
- Fixed the phone remote keeping an old version of itself after Beacon was updated, often for most of a day, so a changed layout simply did not arrive
- Fixed "Clear cache" in Settings leaving most of the cache behind: it spared cover art, artist photos and radio station logos, by a wide margin the largest part of it. It now clears all of them, and reports success once they are actually gone rather than the moment it starts
- Fixed the lyrics drawer coming up empty the first time it was opened after starting Beacon, claiming the song had no lyrics until the Now Playing screen had been visited once
- Fixed the filter box on Songs, Albums, Artists, Genres and Playlists not finding a title, artist or album when the typed text left out an accent the source used, such as searching "revolution" for a song named "La Rèvolution". Works the other way round too
- Fixed the personalized recommendations toggle missing from Settings altogether for accounts that aren't allowed to trigger a library scan, such as a regular non-admin Navidrome account
- Fixed AirPlay speakers being listed as needing to be paired first when they need nothing of the kind, an Apple TV set up without a password and AirPort Express units included
- Fixed a failed cast reporting the speaker's raw technical fault as the whole error message. The cast panel now says which speaker it was and what went wrong: it refused what it was sent, it was busy, or it could not be reached
- Fixed the stream info in the cast device picker claiming a cast radio station was being transcoded to MP3, whatever quality was chosen. Radio is never transcoded by Beacon; the station's own stream is handed straight to the device
- Fixed dragging a track to a new position in the queue on the phone also starting to play it
- Fixed the phone remote zooming in on the page whenever you tapped its search box, leaving the layout at the wrong scale until you pinched back out
- Fixed the track and station labels in the player bar inviting a click that leads nowhere. While an internet radio station is playing neither line has a page of its own to open, so both are plain text now; the artwork beside them still opens the Now Playing screen
- The Now Playing screen on a phone now uses the space it has. The cover was sized well below what fits, filling about two thirds of the width with half the height of the screen left empty
- Fixed the Sign out button in Settings sitting awkwardly on a phone, on a line of its own under the username, so it read as a third line of account text rather than a button
- Fixed the Playlists and Radio pages jumping up and down while something loaded. The spinner sat in the middle of the page and pushed everything below it down, including for background work that had nothing to do with what was on screen
- Settings no longer offers the keyboard shortcuts list on a phone, where there is no keyboard to use any of them with. It is unchanged on the desktop, and in German it is now called "Tastenbelegungen"
