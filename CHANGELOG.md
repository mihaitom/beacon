...
- Playlists can be filled from Last.fm - a country's or genre's charts, an artist's top tracks, tracks similar to one of your own, or your own most played - each looked up in your library and shown before anything is saved, with the option to send the result to the queue instead. Needs a Last.fm key under Settings' Advanced tab
- Playlists can also be filled from ListenBrainz with no key or setup: a genre's top recordings, its charts, an artist's top recordings, your own most listened, or recommendations from your listening history
- Home's two Discover shelves can come from your own listening history instead of from your library's artists, using a public ListenBrainz name; without one they keep using the library-based suggestions
- The lyrics source under the lyrics is now a link to that sheet on lrclib.net, NetEase or SimpMusic
...
- An artist's page now shows the opening paragraph of their Wikipedia article under the name, with a link to the full article; a longer paragraph collapses behind "Show more"
- A track the playlist builder could not find in your library can be copied with one click as "Artist - Title"
- Fanart.tv images now dress the artist surfaces: a background photo on the artist and album pages and behind Now Playing, plus the artist's clear logo in place of the plain name. Beacon ships its own project key so this works with no setup, a switch under Library turns it off, and the images are cached on the server
- The installation-wide keys in Settings now sit together in one "API keys" section
- Where several people share one Beacon, a server administrator can now control who may cast, with a whitelist or blacklist and a separate switch for accounts not on the list

### Changed

- Settings is grouped into tabs - Account, Playback, Library, Casting and Advanced - instead of one long scroll, with Casting and Advanced only where they apply
- The search in the top bar now runs over the library Beacon already holds instead of asking the music server, so it forgives accents, punctuation and word order; a switch beside it turns that off for an exact search
- Searching a radio station's played titles forgives accents, punctuation and word order too
- The library filter fields forgive punctuation and apostrophes the same way
- Clicking a title in a radio station's played list now opens the search on that song's artist and title
- Now Playing's visualizer bars take their colour from the artist background where Fanart.tv has one, and carry a soft shadow to stay legible over a bright photo
- Picking a lyrics match, or letting the automatic lookup run, no longer asks the lyrics service a second time for a sheet its search had already handed over
- The A-Z jump bar now highlights the letter you are in as you scroll, and the Albums and Artists grids draw a lettered divider before each section
...
- An artist whose name is only part of another artist's name could be looked up as that other artist, handing back the wrong Wikipedia paragraph, links and artist image; it now prefers an exact name match
