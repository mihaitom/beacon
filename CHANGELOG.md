...
- Surround tracks, a 5.1 FLAC for instance, stayed completely silent when cast. They are now folded down to stereo for the speaker, which is what every cast target can actually play, and the stream info names the source's channel count so the downmix has a visible reason. Folding five channels into two costs some volume, so a surround track plays a little quieter than a stereo one. That is the standard behaviour of ffmpeg, the audio engine Beacon uses, rather than a level Beacon chooses: it leaves enough headroom that the fold can never distort
