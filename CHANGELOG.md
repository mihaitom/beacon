...
- The volume slider on a phone often ignored the finger while casting to a speaker: it moved a few percent and stopped, or did not move at all, and roughly every second attempt failed to land on the level that was set. Only the thin coloured bar itself took the gesture, so a finger that landed anywhere else on the control was treated as a page scroll and the drag was taken away mid-move. The whole control now belongs to the slider. A drag the page still manages to take also no longer leaves the slider running loose behind it, where later swipes anywhere on the screen went on changing the speaker's level
