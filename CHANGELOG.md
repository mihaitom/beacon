...
- Fixed casting an internet radio station in a format other than MP3 (AAC in particular) failing outright on Sonos with an unsupported-format error. Beacon was telling every device to expect MP3 regardless of the station's real format; it now goes by the stream URL's own file extension when casting starts, so an AAC station is announced as AAC, an Ogg station as Ogg, and so on
