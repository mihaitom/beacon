 ...
 * stores/playback.ts. Both call begin() once up front (before their first
 * await) and hold onto the returned token; after any await, isCurrent(token)
 * tells them whether a newer call has since superseded them, so a
 * slow-to-resolve older call can't stomp state a newer, already-successful
 * one already moved on from. */
