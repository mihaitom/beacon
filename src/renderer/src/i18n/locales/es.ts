import type de from './de'

export default {
  common: {
    cancel: 'Cancelar',
    close: 'Cerrar',
    loading: 'Cargando',
    save: 'Guardar',
    edit: 'Editar',
    add: 'Añadir',
    create: 'Crear',
    delete: 'Eliminar',
    done: 'Hecho',
    retry: 'Reintentar',
    loadMore: 'Cargar más',
    name: 'Nombre',
    addToQueue: 'Añadir a la cola',
    addToPlaylistMenu: 'Añadir a la lista de reproducción',
    createNewPlaylist: 'Crear nueva lista de reproducción…',
    undo: 'Deshacer',
  },
  nav: {
    nowPlaying: 'Reproducción',
    home: 'Inicio',
    library: 'Biblioteca',
    albums: 'Álbumes',
    artists: 'Artistas',
    songs: 'Canciones',
    genres: 'Géneros',
    playlists: 'Listas',
    radio: 'Radio',
    favorites: 'Favoritos',
    stats: 'Estadísticas',
    settings: 'Ajustes',
    back: 'Atrás',
    forward: 'Adelante',
    collapseSidebar: 'Contraer la barra lateral',
    expandSidebar: 'Expandir la barra lateral',
  },
  auth: {
    welcomeBack: 'Bienvenido de nuevo',
    chooseServer: 'Elige tu servidor',
    serverTypeSubsonic: 'Navidrome',
    serverTypeJellyfin: 'Jellyfin',
    serverTypePlex: 'Plex',
    comingSoon: 'Próximamente',
    serverUrl: 'URL del servidor',
    serverUrlSubsonic: 'URL de Navidrome',
    serverUrlJellyfin: 'URL de Jellyfin',
    serverLocked: 'Servidor: {url}',
    username: 'Usuario',
    password: 'Contraseña',
    login: 'Iniciar sesión',
    quickConnectHint:
      'Solicita un código e introdúcelo en otro dispositivo en el que ya hayas iniciado sesión (o en la propia interfaz web de Jellyfin, en Perfil → Conexión rápida) para iniciar sesión aquí.',
    quickConnectApproveHint: 'Introduce este código en otro dispositivo para iniciar sesión:',
    quickConnectRequestCode: 'Solicitar código',
    quickConnectCopyCode: 'Copiar código',
    quickConnectCodeCopied: 'Copiado',
    plexHint:
      'Te inicia sesión a través de tu cuenta de Plex: se abrirá una pestaña del navegador donde inicias sesión en Plex y apruebas la solicitud.',
    plexSignIn: 'Iniciar sesión con Plex',
    plexWaitingHint:
      'Aprueba el inicio de sesión en la pestaña del navegador que se acaba de abrir.',
    plexChooseServer: 'Elige tu servidor Plex:',
    plexNoServers: 'No se encontraron servidores para esta cuenta de Plex.',
  },
  home: {
    frequentlyPlayed: 'Reproducido con frecuencia',
    topSongs: 'Canciones más reproducidas',
    recentlyAdded: 'Añadido recientemente',
    recentlyPlayed: 'Reproducido recientemente',
    rediscover: 'Para redescubrir',
    discover: 'Descubrir en tu biblioteca',
    reroll: 'Volver a mezclar',
    playAll: 'Reproducir todo',
    readyToPlay: '¿Listo para escuchar algo?',
    nothingHeardYet: 'Todavía no has escuchado nada - empieza donde quieras.',
    nowPlaying: 'Reproduciendo ahora',
    paused: 'Pausa',
    keepListening: 'Sigue escuchando',
    radioEyebrow: 'Radio',
    nextUp: 'A continuación',
    internetRadio: 'Radio por internet',
    nothingToShow: 'Nada que mostrar.',
    newArtistsTitle: 'Nuevos artistas por descubrir',
  },
  library: {
    showAsGrid: 'Ver en cuadrícula',
    album: 'Álbum',
    artist: 'Artista',
    playlist: 'Lista de reproducción',
    albums: 'Álbumes',
    artists: 'Artistas',
    songs: 'Canciones',
    genres: 'Géneros',
    genre: 'Género',
    noAlbumsFound: 'No se encontraron álbumes.',
    noAlbumsForQuery: 'No hay álbumes para "{query}".',
    noArtistsFound: 'No se encontraron artistas.',
    noArtistsForQuery: 'No hay artistas para "{query}".',
    noGenresFound: 'No se encontraron géneros.',
    noGenresForQuery: 'No hay géneros para "{query}".',
    noSongsFound: 'No se encontraron canciones.',
    searchSongs: 'Buscar canciones…',
    searchAlbums: 'Buscar álbumes…',
    noSongsForQuery: 'No hay canciones para "{query}".',
    album1: 'álbum',
    albumsN: 'álbumes',
    song1: 'canción',
    songsN: 'canciones',
    songCount: '{count} canciones',
    albumsAndSongs: '{albums} álbumes · {songs} pistas',
    mostPlayed: 'Más reproducidos',
    newestFirst: 'Más recientes primero',
    oldestFirst: 'Más antiguos primero',
    showAllSongs: 'Mostrar todo',
    showMore: 'Mostrar más',
    showLess: 'Mostrar menos',
    allSongs: 'Todas las canciones',
    disc: 'Disco {number}',
    title: 'Título',
    year: 'Año',
    plays: 'Escuchas',
    rating: 'Valoración',
    format: 'Formato',
    duration: 'Duración',
    cover: 'Carátula',
    columns: 'Columnas',
    resetColumns: 'Restablecer valores predeterminados',
    columnNotReported: 'Tu servidor no lo informa',
    sampleRate: 'Frecuencia',
    size: 'Tamaño',
    lastPlayed: 'Última vez',
    play: 'Reproducir',
    playShuffled: 'Aleatorio',
    // MusicBrainz's release types, as the server sends them (lower-case;
    // 'dj-mix' looked up as djmix).
    releaseTypes: {
      album: 'Álbum',
      single: 'Sencillo',
      ep: 'EP',
      compilation: 'Recopilatorio',
      live: 'En directo',
      soundtrack: 'Banda sonora',
      remix: 'Remix',
      broadcast: 'Emisión',
      mixtape: 'Mixtape',
      djmix: 'Mezcla de DJ',
      demo: 'Demo',
    },
    reissued: 'Reedición {year}',
    playRandom: 'Reproducir aleatoriamente',
    playFromTopPlayed: 'Aleatorio de lo más reproducido',
    playNext: 'Reproducir a continuación',
    playAll: 'Reproducir todo',
    songRadio: 'Radio de la canción',
    removeFromPlaylist: 'Quitar de la lista',
    showArtwork: 'Ver imagen',
    songInfo: 'Información',
    goToAlbum: 'Ir al álbum',
    goToArtist: 'Ir al artista',
    menuPlayback: 'Reproducción',
    menuLibrary: 'Biblioteca',
    menuNavigation: 'Navegación',
    menuDetails: 'Detalles',
    songsUnavailable: 'No se pudieron cargar las canciones.',
    songRadioError: 'No se pudo iniciar la radio de la canción.',
    artistRadio: 'Radio del artista',
    artistRadioError: 'No se pudo iniciar la radio del artista.',
    plexPassRequired:
      'Esto necesita una suscripción activa a Plex Pass - Plex bloquea su propia función Sonic Analysis (Radio de canción/artista, Autoplay) detrás de eso, Beacon no puede evitarlo.',
    viewOnService: 'Ver en {service}',
    selected: 'seleccionado(s)',
    selectAll: 'Seleccionar todo',
    selectNone: 'Borrar selección',
    star: 'Añadir a favoritos',
    unstar: 'Quitar de favoritos',
  },
  // Labels for the track-info sheet (SongInfoDialog.vue). Which of these
  // a reader ever sees depends on their own media server: the dialog only
  // shows the fields the server actually answered with (see
  // services/library/songDetails.ts).
  songInfo: {
    title: 'Título',
    sortName: 'Nombre de orden',
    artist: 'Artista',
    albumArtist: 'Artista del álbum',
    album: 'Álbum',
    track: 'Pista',
    disc: 'Disco',
    year: 'Año',
    genre: 'Género',
    bpm: 'BPM',
    mood: 'Estado de ánimo',
    comment: 'Comentario',
    explicit: 'Clasificación de contenido',
    duration: 'Duración',
    format: 'Formato',
    contentType: 'Tipo de medio',
    bitrate: 'Tasa de bits',
    sampleRate: 'Frecuencia de muestreo',
    bitDepth: 'Profundidad de bits',
    channels: 'Canales',
    size: 'Tamaño del archivo',
    path: 'Ruta',
    playCount: 'Reproducciones',
    lastPlayed: 'Última reproducción',
    added: 'Añadido',
    rating: 'Valoración',
    replayGainTrack: 'ReplayGain (pista)',
    replayGainAlbum: 'ReplayGain (álbum)',
    musicBrainzId: 'ID de MusicBrainz',
    isrc: 'ISRC',
    sectionTrack: 'Pista',
    sectionAudio: 'Audio',
    sectionLibrary: 'Biblioteca',
    sectionIds: 'Identificadores',
    error: 'No se han podido cargar los detalles de esta pista.',
    empty: 'Tu servidor no tiene nada sobre esta pista más allá de lo que ya muestra la lista.',
  },
  player: {
    nothingPlaying: 'Nada reproduciéndose',
    mute: 'Silenciar',
    volume: 'Volumen',
    autoplay: 'Reproducción automática',
    liveRadio: 'En directo · {time}',
    live: 'En directo',
    radioConnectionLost: 'Conexión perdida',
    radioReconnect: 'Reconectar',
  },
  shortcuts: {
    title: 'Atajos de teclado',
    or: 'o',
    togglePlay: 'Reproducir / pausar',
    seek: 'Retroceder / avanzar 5 segundos',
    skipTrack: 'Pista anterior / siguiente',
    volume: 'Subir / bajar el volumen',
    mute: 'Silenciar',
    shuffle: 'Alternar reproducción aleatoria',
    repeat: 'Cambiar el modo de repetición',
    favorite: 'Marcar la pista actual como favorita',
    queue: 'Mostrar / ocultar la cola',
    navigate: 'Página anterior / siguiente',
    jump: 'Saltar al 0-90 % de la pista',
    help: 'Mostrar esta lista',
    hint: 'Los atajos se desactivan mientras escribes o hay un diálogo abierto.',
  },
  nowPlaying: {
    nothingPlaying: 'No se está reproduciendo nada ahora mismo.',
    toggleVisualizer: 'Mostrar/ocultar visualizador',
    toggleArtwork: 'Mostrar/ocultar la portada',
    nextBackground: 'Mostrar otro fondo',
    toggleFullscreen: 'Alternar pantalla completa',
    reducedMotionToastTitle: 'Visualizador',
    reducedMotionToastMessage:
      'No se muestra porque tu sistema tiene activada la opción "Reducir movimiento".',
  },
  lyrics: {
    resumeAutoscroll: 'Volver a seguir la canción',
    title: 'Letra',
    notFound: 'No se encontró letra para esta canción.',
    searching: 'Buscando la letra …',
    seekHere: 'Saltar a esta línea',
    sync: 'Sincronizar',
    syncEarlier: 'Adelantar letra',
    syncLater: 'Retrasar letra',
    syncReset: 'Restablecer desfase de sincronización',
    earlierBy: '{seconds}s antes',
    laterBy: '{seconds}s después',
    calibrate: 'Calibrar con una línea',
    calibrateHint: 'Haz clic en la línea que se está cantando ahora mismo',
    calibrateHere: 'Calibrar con esta línea',
    source: 'Fuente: {source}',
    sourceFile: 'Del archivo',
    pickMatch: 'Coincidencias',
    noCandidates: 'No se encontraron más coincidencias.',
    synced: 'Sincronizada',
    unsynced: 'No sincronizada',
    syncUnknown: 'Sincronización desconocida',
    currentMatch: 'En uso actualmente',
  },
  connect: {
    title: 'Transmisión',
    playingOn: 'Reproduciendo en',
    thisDevice: 'Este dispositivo',
    rescan: 'Volver a escanear',
    stopAll: 'Detener todo',
    connect: 'Conectar',
    apply: 'Aplicar',
    interruptedTitle: 'Reproducción interrumpida',
    interrupted: 'El dispositivo cerró la conexión.',
    interruptedOn: '{device} cerró la conexión.',
    interruptedResume: 'Reanudar',
    interruptedResumeFailed: 'No se pudo reanudar la reproducción.',
    searchingDevices: 'Buscando dispositivos …',
    noDevicesFound: 'No se encontraron dispositivos.',
    inUseBy: 'En uso por {name}',
    inUseByUnknown: 'En uso por otra persona',
    takeOver: 'Tomar el control',
    pair: 'Emparejar',
    apiUnreachable:
      'El backend de Connect no está disponible. ¿Está en ejecución y es correcta la URL en Ajustes?',
    authError: 'La sesión de Connect no está autenticada. Vuelve a iniciar sesión.',
    ffmpegMissing: 'Falta ffmpeg en el backend de Connect - la transmisión no funcionará sin él.',
    unknownError: 'Error desconocido.',
    // Keyed by connect/delivery/errors.py's REASON_* constants - a
    // dispatch that reached the device and didn't start playing.
    castForbidden: 'Esta cuenta no puede emitir a los altavoces de este servidor.',
    deliveryFailed: {
      rejected:
        '«{device}» rechazó esta transmisión. El dispositivo no puede reproducir ese formato o esa dirección.',
      busy: '«{device}» está ocupado con otra cosa y rechazó iniciar la reproducción.',
      unreachable: 'No se pudo contactar con «{device}». ¿Está encendido y en la misma red?',
      needs_pairing:
        '«{device}» tiene que emparejarse primero. Emparéjalo desde la lista de dispositivos y vuelve a intentarlo.',
      station_refused:
        'La propia emisora rechazó la conexión. Beacon no pudo ni pasarla a «{device}» ni convertirla; probablemente la dirección de la emisora ya no sea válida',
      unknown: 'Falló la reproducción en «{device}».',
    },
    pairDeviceTitle: 'Emparejar "{name}"',
    pairPinPrompt: 'El dispositivo muestra un PIN - introdúcelo:',
    pairConfirmPrompt: 'Confirma en el dispositivo y luego haz clic en "Hecho".',
    pin: 'PIN',
    takeoverTitle: '¿Tomar el control del dispositivo?',
    takeoverBody:
      '"{name}" está siendo usado actualmente por {owner}. Tomar el control detendrá su reproducción en este dispositivo.',
    streamInfo: {
      title: 'Información del stream',
      transcoding: 'Transcodificación',
      no: 'No, copia directa',
      source: 'Fuente',
      connection: 'Conexión',
      connected: 'Conectado',
      idle: 'Sin conexión activa',
      serverLag: 'Capacidad de respuesta del servidor',
      reason: 'Motivo',
      // Deliberately a couple of words at most: this is the value in a
      // two-column row, so anything longer wraps the row into a block of
      // text. The matching `reasons` sentence below is the hover title.
      reasonsShort: {
        device_limit: 'Límite del dispositivo',
        lossless_container: 'Formato del contenedor',
        codec_not_castable: 'Códec no admitido',
        codec_unknown: 'Códec desconocido',
        probe_failed: 'Formato desconocido',
        replay_gain: 'ReplayGain',
        quality_limit: 'Límite de calidad',
        browser_unsupported: 'Formato del navegador',
        device_rejected_stream: 'Rechazado',
        relay_format_limit: 'Convertida para el dispositivo',
      },
      reasons: {
        device_limit: 'La fuente supera la calidad que admite este dispositivo',
        lossless_container: 'Fuente sin pérdidas en un formato que los dispositivos no aceptan',
        codec_not_castable: 'Este dispositivo no reproduce el códec de origen',
        codec_unknown: 'No se reconoció el códec de origen',
        probe_failed: 'No se pudo detectar el formato de origen',
        replay_gain: 'ReplayGain está activo, por lo que no se puede enviar el archivo tal cual',
        quality_limit: 'La fuente supera el límite de calidad fijado en los ajustes',
        browser_unsupported: 'El navegador no puede reproducir el formato de origen',
        device_rejected_stream:
          'El dispositivo rechazó la transmisión original de la emisora, así que Beacon la convierte',
        relay_format_limit:
          'La emisora no envía un formato que pueda entregarse tal cual, por lo que Beacon la convierte; en Calidad de audio puedes elegir AAC en lugar de MP3',
      },
    },
  },
  queue: {
    title: 'Cola',
    empty: 'La cola está vacía.',
    clear: 'Vaciar cola',
    saveAsPlaylist: 'Guardar cola como lista de reproducción',
  },
  remoteControl: {
    title: 'Control remoto',
    hint: 'Permite que un teléfono en tu red controle la reproducción - Reproduciendo ahora, Cola, Listas de reproducción, Canciones y Radio.',
    enableFailed: 'No se pudo activar el control remoto.',
    disableFailed: 'No se pudo desactivar el control remoto.',
    pairTitle: 'Emparejar un teléfono',
    pairHint:
      'Escanea este código con la cámara de tu teléfono, o abre la dirección de abajo e introduce el PIN.',
    address: 'Dirección',
    turnOff: 'Desactivar',
    regenerate: 'Regenerar código',
    needsRegenerate:
      'El código de emparejamiento anterior ya no está disponible. Genera uno nuevo para emparejar un teléfono - esto desconectará cualquier teléfono emparejado anteriormente.',
  },
  settings: {
    title: 'Ajustes',
    account: 'Cuenta',
    logout: 'Cerrar sesión',
    language: 'Idioma',
    libraryTitle: 'Biblioteca',
    libraryScanHint: 'Haz que {server} busque archivos nuevos, modificados o eliminados.',
    rescanLibrary: 'Volver a escanear biblioteca',
    scanning: 'Escaneando … ({count})',
    scanningPercent: 'Analizando \u2026 ({percent}%)',
    scanningPlain: 'Analizando \u2026',
    scanComplete: 'Escaneo completo - {count} canciones procesadas.',
    scanFailed: 'No se pudo iniciar el escaneo.',
    libraryRefreshHint:
      'Recarga la vista de biblioteca propia de Beacon ahora, en lugar de esperar a la actualización automática.',
    refreshLibrary: 'Actualizar biblioteca',
    refreshingLibrary: 'Actualizando … ({loaded})',
    refreshingLibraryWithTotal: 'Actualizando … ({loaded} / {total})',
    libraryRefreshed: 'Biblioteca actualizada - {count} canciones cargadas.',
    refreshLibraryFailed: 'No se pudo actualizar la biblioteca.',
    castPermissionsTitle: 'Emisión',
    castPermissionsHint:
      'Decide quién puede emitir a los altavoces de este servidor. Todos pueden seguir escuchando en local.',
    castPermissionsPolicy: '¿Quién puede emitir?',
    castPermissionsPolicyEveryone: 'Todos',
    castPermissionsPolicyAllowlist: 'Solo las cuentas listadas',
    castPermissionsPolicyBlocklist: 'Todos excepto las cuentas listadas',
    castPermissionsPolicyNobody: 'Nadie excepto los administradores',
    castPermissionsEffectEveryone: 'Todos pueden emitir.',
    castPermissionsEffectOnlyListed: 'Solo las cuentas listadas pueden emitir.',
    castPermissionsEffectAllExceptListed: 'Todos excepto las cuentas listadas pueden emitir.',
    castPermissionsEffectNobody: 'Nadie excepto los administradores puede emitir.',
    castPermissionsAccountAllow: 'Cuentas que pueden emitir',
    castPermissionsAccountBlock: 'Cuentas que no pueden emitir',
    castPermissionsNoUsers: 'Solo se listan las cuentas que ya han iniciado sesión en este Beacon.',
    castPermissionsSave: 'Guardar',
    castPermissionsSaved: 'Permisos de emisión guardados.',
    castPermissionsSaveFailed: 'No se pudieron guardar los permisos de emisión.',
    castPermissionsLoadFailed: 'No se pudieron cargar los permisos de emisión.',
    storageTitle: 'Almacenamiento',
    clearCacheHint:
      'Descarta lo que este navegador guarda en local: datos de biblioteca, carátulas, fotos de artistas, logotipos de emisoras y letras. Todo se vuelve a cargar la próxima vez que se necesite. Solo afecta a este dispositivo: el backend de Connect conserva su propia caché, así que otros dispositivos y cuentas no se tocan. No provoca un escaneo de biblioteca.',
    clearCache: 'Borrar caché',
    cacheCleared: 'Caché borrada.',
    resetAirplayHint:
      'Olvida los emparejamientos de AirPlay guardados (HomePod, Apple TV, …) - útil cuando un dispositivo deja de conectarse. Cada dispositivo deberá emparejarse de nuevo después.',
    resetAirplay: 'Restablecer emparejamientos de AirPlay',
    airplayReset: 'Emparejamientos de AirPlay restablecidos.',
    airplayResetFailed: 'No se pudieron restablecer los emparejamientos de AirPlay.',
    lastfmTitle: 'Last.fm',
    lastfmWhat:
      'Permite a cualquiera que use este Beacon crear listas a partir de las listas de éxitos de Last.fm, de un género, un artista o su propio historial.',
    lastfmStep1: 'Abre last.fm/api/account/create e inicia sesión.',
    lastfmStep2:
      'Rellena un nombre de aplicación y una descripción: vale cualquier cosa razonable, nadie lo revisa.',
    lastfmStep3:
      'Deja vacío el campo «Callback URL»: solo sirve para aplicaciones que inician sesión de usuarios a través de Last.fm, cosa que Beacon no hace. Si el formulario lo exige, sirve cualquier dirección; nunca se usa.',
    lastfmStep4: 'Copia la «API key» que te muestra y pégala aquí. El shared secret no hace falta.',
    fanartTitle: 'Fanart.tv',
    fanartWhat:
      'Añade fondos y logos de artista a las páginas de artista y a «Sonando ahora», de Fanart.tv. Funciona sin configurar nada — Beacon se identifica con su propia clave de proyecto —, pero se recomienda una clave personal gratuita: mantiene las imágenes más frescas y las peticiones en tu propio límite.',
    fanartStep1: 'Crea una cuenta gratuita en fanart.tv.',
    fanartStep2:
      'Abre las claves de API de tu perfil y copia tu clave personal, la vinculada a tu cuenta, no una clave de proyecto.',
    apiKeysTitle: 'Claves de API',
    apiKeyLabel: 'Clave de API de {service}',
    apiKeyGet: 'Conseguir una clave',
    apiKeyPlaceholder: 'Pega aquí la clave',
    apiKeySet: 'Ya hay una clave configurada',
    apiKeyMissing: 'Sin clave.',
    apiKeyStored: 'Hay una clave configurada. Introduce otra para sustituirla.',
    apiKeyFromEnvironment: 'Se usa la clave de la configuración de esta instalación.',
    apiKeyClear: 'Quitar clave',
    apiKeySaved: 'Clave guardada',
    apiKeyCleared: 'Clave eliminada',
    apiKeyFailed: 'No se ha podido guardar la clave',
    advancedTitle: 'Avanzado',
    logLevel: 'Nivel de registro',
    logLevelHint:
      'Cuánto detalle escribe el backend de Connect en su propio registro - también afecta lo que aparece en los registros de Docker/contenedor. Se aplica de inmediato, sin necesidad de reiniciar.',
    logLevelTrace: 'Traza',
    logLevelDebug: 'Depuración',
    logLevelInfo: 'Información',
    logLevelWarning: 'Advertencia',
    logLevelError: 'Error',
    logLevelChanged: 'Nivel de registro actualizado.',
    logLevelChangeFailed: 'No se pudo actualizar el nivel de registro.',
    recommendations: 'Recomendaciones personalizadas',
    recommendationsHint:
      'Descubrir en Inicio usa artistas parecidos a lo que escuchas realmente, consultados en MusicBrainz, ListenBrainz y Deezer - esto comparte con ellos uno o dos nombres de artista de tu biblioteca. Desactivado muestra álbumes al azar; abrir la página de un artista sigue consultando a ese artista en cualquier caso.',
    fanartEnabled: 'Imágenes de artistas de Fanart.tv',
    fanartEnabledHint:
      'Muestra los fondos y logos de artista de Fanart.tv en las páginas de artista y en «Sonando ahora». Beacon se identifica con su propia clave de proyecto, así que funciona sin configurar nada; una clave personal mantiene las peticiones en tu propio límite.',
    listenbrainzUsername: 'Usuario de ListenBrainz',
    listenbrainzHint:
      'Tu nombre público de ListenBrainz. Rellena la estantería «Recomendado para ti» en Inicio y se ofrece como valor predeterminado en el generador de listas. No hace falta contraseña. Para crear una lista, eso sí, Last.fm es la mejor opción: sus listas son más variadas.',
    lyricsProvidersTitle: 'Proveedores de letras',
    lyricsProvidersHint:
      'Las letras guardadas en el propio archivo se intentan siempre primero y nunca salen de tu servidor. Todos los proveedores de abajo están activados por defecto - desmarca aquellos a los que prefieras que Beacon no envíe el título y el artista de una canción.',
    lyricsProviders: 'Proveedores externos',
    lyricsProvidersEmptyHint:
      'Ningún proveedor seleccionado - solo se muestran las letras guardadas en el propio archivo.',
    lyricsProvidersActiveHint:
      'Si el archivo no tiene letra, se envían el título y el artista a los proveedores seleccionados para buscar una coincidencia. El álbum y la duración nunca salen de tu servidor; solo se usan aquí, para elegir el resultado más parecido.',
    playbackTitle: 'Reproducción',
    replayGain: 'ReplayGain',
    replayGainOff: 'Desactivado',
    replayGainTrack: 'Pista',
    replayGainAlbum: 'Álbum',
    replayGainHint:
      'Iguala el volumen de grabaciones masterizadas de forma diferente - «Pista» normaliza cada canción individualmente, «Álbum» conserva las diferencias de volumen dentro de un álbum. Se aplica tanto a la reproducción local como a la transmisión; mientras ya se está transmitiendo, un cambio solo tiene efecto a partir de la siguiente pista.',
    replayGainMobileHint:
      'En este dispositivo se aplica solo a la transmisión: la reproducción aquí pasa directamente por el navegador para que siga sonando con la pantalla bloqueada, lo que no deja forma de ajustar el volumen del audio por el camino.',
    localQuality: 'Calidad de audio (este dispositivo)',
    localQualityHint:
      'Solo se aplica a la reproducción en este dispositivo y se guarda aquí, no en la cuenta, así que un móvil y un ordenador pueden tener ajustes distintos. «Original» envía el archivo sin modificar; cualquier otro formato se convierte al reproducirse, lo que ahorra ancho de banda y permite reproducir archivos que el navegador no puede abrir. Solo se ofrecen los formatos que este navegador sabe descodificar. El cambio surte efecto a partir de la siguiente pista.',
    castQuality: 'Calidad de audio (transmisión)',
    castQualityHint:
      'Un límite máximo, no una elección fija: Beacon sigue eligiendo el mejor formato que acepte cada dispositivo, pero nunca envía nada por encima de este ajuste. Una grabación que ya cabe por debajo se envía sin modificar. Los límites del propio dispositivo se aplican siempre además de este. Surte efecto a partir de la siguiente pista.',
    qualityFormat: 'Formato',
    qualityBitrate: 'Tasa de bits',
    qualityBitrateItem: '{value} kb/s',
    qualityOriginal: 'Original (sin modificar)',
    qualityTipsTitle: 'Recomendaciones',
    qualityTips: {
      ceiling:
        'Es un límite máximo, no una conversión: lo que ya está por debajo se reproduce sin modificar.',
      home: 'En casa con wifi: Original, así no se convierte nada.',
      mobile: 'Fuera de casa: AAC a 192 kbps suena como MP3 a 320 y consume muchos menos datos.',
      slow: 'Con conexión lenta: Opus a 96 o 128 kbps es lo más pequeño con la misma calidad.',
      compatibility:
        'MP3 a 256 o 320 kbps es el que se reproduce en todas partes y la respuesta cuando algo más da problemas.',
      castDefault:
        'El altavoz está en la misma red que Beacon, así que aquí la cuestión no es el tamaño de los datos: «Original» suele ser lo correcto.',
      castWhenNeeded:
        'Poner un límite compensa cuando algo falla: wifi débil en esa habitación o un dispositivo que se atasca con archivos sin pérdida grandes. AAC a 256 kbps sobra.',
      castCompatibility:
        'MP3 a 320 kbps es la respuesta en cuanto un dispositivo da algún problema: se reproduce en todas partes.',
      castDevice:
        'Beacon lo reduce a lo que el altavoz reproduce realmente: a un dispositivo sin Opus se le envía AAC, o MP3 si tampoco lo acepta, con la tasa de bits que hayas elegido.',
    },
    castRadioDirectly: 'Reproducir la radio directamente desde la emisora',
    castRadioDirectlyHint:
      'Por defecto, Beacon enruta la radio a través de su propio backend, tanto para un dispositivo de emisión como para este reproductor. Una sola conexión a la emisora alimenta el audio y el título en reproducción, y si la emisora se cae Beacon vuelve a conectarse sin interrumpir la reproducción aquí. Activado, el dispositivo y este reproductor se conectan directamente a la emisora: la reproducción sigue aunque Beacon se reinicie, pero una caída solo se nota cuando desaparece el sonido, y algunos dispositivos rechazan la emisión de la emisora.',
    about: 'Acerca de Beacon',
    whatsNew: '¿Qué hay de nuevo?',
    ffmpegFound: 'ffmpeg encontrado',
    ffmpegMissing: 'Falta ffmpeg - la transmisión no funcionará sin él.',
    version: 'Versión {version}',
    updateAvailable: 'La versión {version} está disponible.',
    updateAvailableLink: 'Ver versión',
  },
  lastfm: {
    title: 'Lista de Last.fm',
    source: 'Rellenar con',
    sourceCharts: 'Listas de éxitos',
    sourceGenre: 'Top de un género',
    sourceArtist: 'Top de un artista',
    sourceSimilar: 'Pistas similares',
    sourceMyTop: 'Lo que más escucho',
    chartScope: 'Lista',
    chartCountry: 'Un país',
    chartGlobal: 'Mundial',
    country: 'País',
    countryPlaceholder: 'Elige un país',
    genre: 'Género',
    genrePlaceholder: 'rock, jazz, techno…',
    artist: 'Artista',
    seedTrack: 'Pista',
    seedTrackPlaceholder: 'Buscar en tu biblioteca…',
    seedHint: 'Escribe para buscar en tu biblioteca',
    seedNoResults: 'Sin coincidencias',
    username: 'Usuario de Last.fm',
    usernameHint: 'Tu nombre público de Last.fm. No hace falta contraseña.',
    period: 'Periodo',
    period7day: 'Últimos 7 días',
    period1month: 'Último mes',
    period3month: 'Últimos 3 meses',
    period6month: 'Últimos 6 meses',
    period12month: 'Último año',
    periodOverall: 'Desde siempre',
    limit: 'Canciones',
    search: 'Buscar en la biblioteca',
    back: 'Atrás',
    fetching: 'Consultando Last.fm…',
    matching: 'Buscando en tu biblioteca ({done} de {total})',
    eyebrow: 'Generador de listas',
    summaryFound: 'canciones encontradas en tu biblioteca',
    summaryHint: 'Revisa las parejas de abajo antes de guardar. El resto no está en tu biblioteca.',
    existingPlaylist: '«{name}» ya existe.',
    modeAppend: 'Añadir lo que falta y conservar el resto',
    modeReplace: 'Sustituir su contenido por esta lista',
    appendAction: 'Añadir',
    replaceAction: 'Sustituir',
    columnLastfm: 'Last.fm',
    columnLibrary: 'Tu biblioteca',
    closeMatch: 'coincidencia aproximada',
    missing: 'no está en la biblioteca',
    copyTrack: 'Copiar pista',
    noTracks: 'Last.fm no ha devuelto nada.',
    failed: 'No se ha podido conectar con Last.fm.',
    createFailed: 'No se ha podido crear la lista.',
    playFailed: 'No se ha podido iniciar la reproducción.',
    queueMenu: 'Cola',
    newQueue: 'Sustituir la cola',
    appendQueue: 'Añadir a la cola',
    nameGlobalCharts: 'Last.fm éxitos mundiales',
    nameCountryCharts: 'Last.fm éxitos {country}',
    nameGenre: 'Last.fm {tag}',
    nameArtist: 'Last.fm {artist}',
    nameSimilar: 'Similar a {title}',
    nameMyTop: 'Lo que más escucho ({period})',
  },
  listenbrainz: {
    title: 'Lista de ListenBrainz',
    sourceCharts: 'Listas de éxitos',
    sourceGenre: 'Top de un género',
    sourceArtist: 'Top de un artista',
    sourceMyTop: 'Lo que más escucho',
    sourceRecommended: 'Recomendado para mí',
    username: 'Usuario de ListenBrainz',
    usernameHint: 'Tu nombre público de ListenBrainz. No hace falta contraseña.',
    period: 'Periodo',
    periodWeek: 'Última semana',
    periodMonth: 'Último mes',
    periodQuarter: 'Últimos 3 meses',
    periodHalfYearly: 'Últimos 6 meses',
    periodYear: 'Último año',
    periodAllTime: 'Desde siempre',
    columnListenbrainz: 'ListenBrainz',
    noTracks: 'ListenBrainz no ha devuelto nada.',
    failed: 'No se ha podido conectar con ListenBrainz.',
    fetching: 'Consultando ListenBrainz…',
    nameCharts: 'ListenBrainz éxitos',
    nameGenre: 'ListenBrainz {tag}',
    nameArtist: 'ListenBrainz {artist}',
    nameMyTop: 'Lo que más escucho ({period})',
    nameRecommended: 'Recomendado para mí',
  },
  playlists: {
    reorderFailed: 'No se pudo guardar el nuevo orden',
    removedOne: 'Se quitó «{title}»',
    removedMany: 'Se quitaron {count} pistas',
    removeFailed: 'No se pudo quitar de la lista de reproducción',
    restoreFailed: 'No se pudo restaurar la lista de reproducción',
    title: 'Listas de reproducción',
    playlist1: 'lista de reproducción',
    playlistsN: 'listas de reproducción',
    newPlaylist: 'Nueva',
    createTitle: 'Nueva lista de reproducción',
    noPlaylistsYet: 'Todavía no hay listas de reproducción.',
    noSongsYet: 'Esta lista de reproducción está vacía.',
    created: '“{name}” creada',
    addedOne: 'Añadido a “{name}”',
    addedMany: '{count} pistas añadidas a “{name}”',
    noPlaylistsForQuery: 'No hay listas de reproducción para "{query}".',
    songCount: '{count} canciones',
    durationHours: '{hours} h {minutes} min',
    durationMinutes: '{minutes} min',
    public: 'Pública',
    personal: 'Mis listas',
    global: 'Listas globales',
    editTitle: 'Editar lista de reproducción',
    byOwner: 'de {owner}',
    deleteTitle: '¿Eliminar lista de reproducción?',
    deleteConfirm: '¿Eliminar "{name}"? Esto no se puede deshacer.',
  },
  privacy: {
    title: 'Privacidad',
    intro:
      'Beacon funciona contra tu propio servidor multimedia. No hay registro, ni analytics, ni tracking, ni anuncios. Aun así, algunas funciones consultan servicios externos. Aquí están cuáles, para qué se consultan y qué se envía con cada petición.',
    fromDeviceTitle: 'Conexiones desde este dispositivo',
    fromDeviceHint:
      'Vale para todo lo de esta sección: tu dispositivo abre la conexión por sí mismo, así que el otro extremo ve tu dirección IP.',
    viaServerTitle: 'Conexiones a través de tu servidor Beacon',
    viaServerHint:
      'Vale para todo lo de esta sección: tu dispositivo solo habla con tu propio servidor Beacon, que hace la petición en su nombre. Por eso el otro extremo ve la dirección de tu servidor, no la tuya.',
    sends: 'Se envía:',
    buildsTitle: 'Aplicación de escritorio y Docker',
    builds:
      'En la versión Docker, tu servidor Beacon se ejecuta donde tú lo alojes: las peticiones de la segunda sección salen entonces desde la dirección de ese servidor, no desde la tuya. La aplicación de escritorio lleva ese mismo servidor dentro y lo arranca en este equipo: ahí ambas son la misma dirección, y la separación de arriba describe el recorrido más que una separación real. También solo en escritorio: descarga por sí misma la actualización que encuentra, en lugar de solo avisar de ella.',
    ownServer:
      'No están en esta lista: tu servidor multimedia (Navidrome, Jellyfin, Plex), tus altavoces y el mando del teléfono. Son tuyos, y Beacon habla con ellos directamente.',
    optOut: {
      lyrics: 'Se puede desactivar en Letras',
      recommendations: 'Se puede desactivar en Recomendaciones',
      recommendationsPartial: 'Parcialmente desactivable en Recomendaciones',
      fanart: 'Se puede desactivar en Ajustes',
    },
    services: {
      updateCheck: {
        name: 'GitHub',
        purpose: 'Comprobar si se ha publicado una versión más reciente de Beacon.',
        sends:
          'Nada sobre ti. Lee la lista de versiones; la comparación ocurre en tu dispositivo. La aplicación de escritorio también descarga la actualización desde ahí.',
      },
      radioStream: {
        name: 'La propia emisora de radio',
        purpose: 'Reproducir el sonido cuando escuchas radio en este dispositivo.',
        sends:
          'Nada más que la petición del stream. La emisora ve tu dirección IP, como cualquier radio web. Al enviar a un altavoz pasa por tu servidor Beacon.',
      },
      radioBrowser: {
        name: 'Radio Browser',
        purpose: 'El directorio de emisoras detrás de «Descubrir emisoras».',
        sends:
          'Tu término de búsqueda y el país elegido. Al reproducir una emisora encontrada aquí, Beacon informa de su identificador como clic; lo piden las propias reglas del directorio, para que las emisoras populares sigan siendo reconocibles. Una emisora que hayas añadido escribiendo su dirección nunca se informa. Al votar por una emisora también se envía su identificador, solo cuando pulsas tú ese botón.',
      },
      stationSite: {
        name: 'Los sitios web de las emisoras',
        purpose:
          'Encontrar el logotipo de una emisora, visible en la lista y durante la reproducción.',
        sends: 'Nada sobre ti. Lee la página de inicio de la emisora y la imagen que indica.',
      },
      lrclib: {
        name: 'LRCLIB',
        purpose: 'Buscar letras cuando el archivo no trae ninguna.',
        sends:
          'El título y el artista de la canción. Su álbum y su duración permanecen en tu servidor; allí solo se usan para elegir el resultado más parecido.',
      },
      netease: {
        name: 'NetEase',
        purpose: 'Buscar letras cuando el archivo no trae ninguna.',
        sends:
          'El título y el artista de la canción. Su álbum y su duración permanecen en tu servidor; allí solo se usan para elegir el resultado más parecido.',
      },
      simpmusic: {
        name: 'SimpMusic',
        purpose: 'Buscar letras cuando el archivo no trae ninguna.',
        sends:
          'El título y el artista de la canción. Su álbum y su duración permanecen en tu servidor; allí solo se usan para elegir el resultado más parecido.',
      },
      musicbrainz: {
        name: 'MusicBrainz',
        purpose:
          'Asocia nombres de artistas a un identificador estable, sobre el que se basan las recomendaciones. También se usa para los enlaces y el párrafo de Wikipedia de la página de un artista, estén o no activadas las recomendaciones.',
        sends: 'Nombres de artistas de tu biblioteca.',
      },
      listenbrainz: {
        name: 'ListenBrainz',
        purpose:
          'Sugerir artistas similares para "Descubrir nuevos artistas" y rellenar una lista con las listas de ListenBrainz, los mejores temas de un artista o tu propio historial de escucha.',
        sends:
          'Para artistas similares, los identificadores de MusicBrainz de los artistas de partida. Para el generador de listas, lo que has pedido: un nombre de artista o el nombre público de ListenBrainz que has introducido. No se envía nada de tu biblioteca; la coincidencia se hace en tu dispositivo.',
      },
      coverartarchive: {
        name: 'Cover Art Archive',
        purpose: 'Mostrar la portada de una grabación recomendada en Inicio.',
        sends:
          'Nada sobre ti. Lee la imagen de portada de un identificador de edición que ha devuelto ListenBrainz.',
      },
      lastfm: {
        name: 'Last.fm',
        purpose:
          'Obtener listas de éxitos, el top de un género o artista, o tu propio historial de escucha, para el generador de listas en la página Listas.',
        sends:
          'Lo que has pedido: un país, un género, un nombre de artista y, para tus más escuchadas, el usuario de Last.fm que has introducido. No se envía nada de tu biblioteca; la comparación ocurre en tu dispositivo.',
      },
      fanart: {
        name: 'Fanart.tv',
        purpose:
          'Busca la imagen de fondo y el logo para la página de artista que tienes abierta, y el fondo tras «Sonando ahora».',
        sends:
          'El nombre del artista cuya página tienes abierta, para buscar antes su id de MusicBrainz. Nada sobre tu biblioteca.',
      },
      deezer: {
        name: 'Deezer',
        purpose:
          'Busca una foto y una página de artista para un artista sugerido, y para la página de artista que tengas abierta, estén o no activadas las recomendaciones.',
        sends: 'El nombre del artista consultado: uno sugerido o aquel cuya página tienes abierta.',
      },
      wikidata: {
        name: 'Wikidata',
        purpose:
          'Encontrar el artículo de Wikipedia del artista cuya página tienes abierta, en tu idioma si existe.',
        sends:
          'El identificador de Wikidata del artista, que proporciona MusicBrainz, y el idioma de la app - ningún nombre.',
      },
      wikipedia: {
        name: 'Wikipedia',
        purpose: 'Mostrar el primer párrafo de ese artículo en la página del artista.',
        sends: 'El título del artículo. El idioma de la app forma parte de la dirección.',
      },
      plexAuth: {
        name: 'Plex',
        purpose:
          'Solo si inicias sesión con una cuenta de Plex: Plex gestiona el inicio de sesión a través de plex.tv, no de tu propio servidor.',
        sends:
          'Una petición de inicio de sesión y, después, tu token de Plex cada vez que se comprueba. Para iniciar sesión se abre app.plex.tv en el navegador. Con Navidrome o Jellyfin no ocurre nada de esto.',
      },
    },
  },
  radio: {
    title: 'Radio',
    station1: 'emisora',
    stationsN: 'emisoras',
    addStation: 'Añadir emisora',
    addFailedTitle: 'Emisora no guardada',
    addFailedMessage:
      'El servidor multimedia rechazó guardarla. En algunos servidores, gestionar emisoras de radio requiere una cuenta de administrador.',
    discoverStations: 'Descubrir emisoras',
    discoverTitle: 'Descubrir emisoras de radio',
    discoverSearchLabel: 'Buscar emisoras',
    discoverSearchPlaceholder: 'p. ej. jazz, BBC, France Inter...',
    discoverNoResults: 'No se encontraron emisoras para "{query}".',
    discoverNoResultsFiltered: 'No se encontraron emisoras para estos filtros.',
    discoverSearchFailed:
      'No se pudo conectar con el directorio de emisoras. Inténtalo de nuevo en un momento.',
    discoverAdded: 'Añadida',
    discoverCountryLabel: 'País',
    discoverAnyCountry: 'Cualquier país',
    discoverTopVoted: 'Más votadas',
    discoverMostPlayed: 'Más escuchadas',
    discoverColumnLocation: 'Ubicación',
    discoverColumnLanguage: 'Idioma',
    discoverColumnCodec: 'Códec',
    discoverColumnVotes: 'Votos',
    discoverColumnClicks: 'Clics (24h)',
    discoverVote: 'Votar por esta emisora',
    discoverVoted: 'Has votado por esta emisora',
    discoverVoteLimitTitle: 'Ya has votado hoy',
    discoverVoteLimitMessage:
      'Radio Browser cuenta un voto por emisora y día, para todos los que comparten este servidor de Beacon.',
    discoverVoteFailedTitle: 'Voto no enviado',
    discoverVoteFailedMessage:
      'No se pudo conectar con Radio Browser. Inténtalo de nuevo más tarde.',
    discoverStreamOk: 'En línea en la última comprobación',
    discoverStreamDown: 'Fuera de línea en la última comprobación',
    discoverPlay: 'Reproducir sin añadir',
    discoverCredit: 'Datos de emisoras de Radio Browser',
    createTitle: 'Nueva emisora de radio',
    editTitle: 'Editar emisora de radio',
    streamUrl: 'URL de transmisión',
    homePageUrl: 'URL de la página principal',
    noStationsYet: 'Todavía no hay emisoras de radio guardadas.',
    noStationsForQuery: 'No hay emisoras para "{query}".',
    titleLog: 'Historial de títulos',
    titleLogEmpty: 'Nada emitido aún desde que empezó esta emisora.',
    titleLogFilter: 'Buscar en el historial',
    titleLogFilterPlaceholder: 'Buscar títulos',
    titleLogFilterEmpty: 'Nada en el historial de esta emisora coincide con «{query}».',
    titleLogSearch: 'Buscar en tu biblioteca',
    titleLogYesterday: 'Ayer',
  },
  favorites: {
    eyebrow: 'Tu colección',
    countArtists: '{n} artista | {n} artistas',
    countAlbums: '{n} álbum | {n} álbumes',
    countSongs: '{n} canción | {n} canciones',
    title: 'Favoritos',
    artists: 'Artistas',
    albums: 'Álbumes',
    songs: 'Canciones',
    noneYet: 'Todavía no se ha marcado ningún favorito.',
  },
  search: {
    label: 'Buscar',
    resultsFor: 'Resultados para "{query}"',
    artists: 'Artistas',
    albums: 'Álbumes',
    songs: 'Canciones',
    exactMatch: 'Ortografía exacta',
    noResults: 'No hay resultados para "{query}".',
  },
  releaseNotes: {
    title: 'Notas de la versión',
    heading: 'Qué ha cambiado desde la última versión',
    subheading: 'Elige una versión a continuación para leer sus cambios aquí mismo.',
    version: 'Versión',
    current: 'Actual: v{version}',
    empty: 'Todavía no hay entradas en el registro de cambios.',
  },
  stats: {
    eyebrow: 'Tu biblioteca',
    title: 'Estadísticas',
    libraryTitle: 'Biblioteca',
    songs: 'Canciones',
    artists: 'Artistas',
    albums: 'Álbumes',
    genres: 'Géneros',
    libraryDuration: 'Tiempo total de reproducción',
    listeningTitle: 'Escucha',
    totalPlays: 'Reproducciones',
    listeningTime: 'Tiempo total de escucha',
    favoriteSongs: 'Canciones favoritas',
    favoriteAlbums: 'Álbumes favoritos',
    favoriteArtists: 'Artistas favoritos',
    noPlaysYet:
      'Todavía no hay reproducciones registradas - esto se completará cuando empieces a escuchar.',
    topSongs: 'Canciones más reproducidas',
    topArtists: 'Artistas más reproducidos',
    topAlbums: 'Álbumes más reproducidos',
    topGenres: 'Géneros más reproducidos',
    formatsTitle: 'Formatos',
    decadesTitle: 'Décadas',
    storage: 'Almacenamiento',
    playedShare: 'de la biblioteca escuchado',
    recentlyPlayed: 'canciones escuchadas en los últimos 30 días',
    favoritesTitle: 'Favoritos y valoraciones',
    ratedSongs: 'Canciones valoradas',
    averageRating: 'Valoración media',
    qualityTitle: 'Calidad de audio',
    quality: {
      hires: 'Hi-Res',
      lossless: 'Sin pérdida (CD)',
      lossy: 'Con pérdida',
    },
    largestArtistsTitle: 'Artistas con más canciones',
    decade: '{decade}s',
    days: '{days}d {hours}h',
    hours: '{hours}h {minutes}m',
    minutes: '{minutes}m',
  },
  mobile: {
    tabNowPlaying: 'Reproduciendo ahora',
    tabQueue: 'Cola',
    playOn: 'Reproducir en',
    needsPairing: 'Necesita emparejarse primero en la aplicación Beacon',
  },
  updateToast: {
    title: 'Actualización disponible',
    electronMessage:
      'La versión {version} se está descargando en segundo plano y se instalará automáticamente la próxima vez que reinicies Beacon.',
    webMessage: 'La versión {version} está disponible.',
    remindLater: 'Recordarme más tarde',
  },
} satisfies typeof de
