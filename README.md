...
| Limit casting to listed accounts (web/Docker) |     ✅ (admins)      | ✅ (admins) |               ❌               |
...
Casting can be controlled per account, so an administrator can share their Beacon without handing over the speakers: the policy is a whitelist or a blacklist, and a second switch decides what accounts not on the list - including ones that show up later - are allowed to do. Anyone who may not cast can still browse and listen locally. The policy is server-wide, kept per media server, the administrator is always allowed, and an untouched server lets everyone cast. It is offered in the Docker and web builds only - the desktop app runs a single-user backend of its own, where there is nobody to restrict - and not on Plex, which has no server-side user list. The picker offers the accounts that have signed in to this Beacon, plus Jellyfin's own full list. The account matched is the one the server reports for the signed-in credentials, not the name typed at login. See [docs/cast-permissions.md](docs/cast-permissions.md) for what this does and does not enforce.
