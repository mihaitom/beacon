...
- **Adjustable backend log level** (Debug/Info/Warning/Error) from Settings, in effect immediately, no restart needed - the `DEBUG` env var remains a fallback for a deployment that never comes up far enough to reach Settings.
...
| `DEBUG`                  | `false`              | Also serves the Connect API's docs at `/api/docs`, unrelated to logging. For log verbosity, prefer the log-level dropdown in Settings (Debug/Info/Warning/Error, persisted, no restart needed) - this env var is only the fallback for a deployment that never comes up far enough to reach it, in which case `true` logs everything (AirPlay, Sonos, the internal streamer, `httpx`/`uvicorn.access`, nginx access logs). A lot of output; leave `false` for normal operation.                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
