...
- **Browse your library** - Albums, Artists, Tracks, Genres, Playlists, Favorites, and search, all with local filtering across the whole library. Track lists let you pick their columns, and artist pages open with the first paragraph of the artist's Wikipedia article.
- **Artist images from Fanart.tv** - a background photo on the artist and album pages and behind Now Playing, the artist's logo in place of the plain name, and banners across the library page headers. Works without setup, can be switched off under Library in Settings, and the images are cached on the Beacon server.
- **A proper Home** - quick access to what you've been listening to, most played tracks, recently added albums, a Rediscover shelf of songs you played a lot but haven't heard in months, and real recommendations: a Discover shelf seeded from artists you actually play (via MusicBrainz + ListenBrainz), or from your own ListenBrainz listening history if you enter your ListenBrainz name, plus a "New to explore" shelf of similar artists not yet in your library, complete with photos and a link out. Toggleable in Settings.
...
- **A playlist builder** - fills a new playlist from Last.fm (country or genre charts, an artist's top tracks, tracks similar to one of yours, your own most played) or ListenBrainz (genre charts, an artist's top recordings, your own most listened, recommendations), using only the tracks your library actually has. ListenBrainz needs no setup; Last.fm needs an API key, see `LASTFM_API_KEY` below.
...
- **Trigger a library rescan** from Settings (administrators only), with a completion notice.
...
| Limit casting to listed accounts (web/Docker) |     ✅ (admins)      | ✅ (admins) |           ✅ (owner)           |

Song/Artist Radio and Autoplay all come from one call (`getSimilarSongs2.view`), and the three servers answer it very differently. Navidrome mixes the artist's own tracks in with similar ones. Jellyfin translates it to its InstantMix, whose quality stands or falls with the metadata: an artist carrying genre tags gets a genuinely matching mix (a Hip Hop artist returned 9 Hip Hop tracks out of 10 against a live library), while an artist without them gets something close to a random draw - and the seeded artist's own tracks never appear either way, so a Jellyfin radio is all discovery and no familiarity. If yours feels arbitrary, the artists are probably untagged; Jellyfin 12 also lets plugins supply similarity data, which is the other way to improve it. Plex needs an active Plex Pass here and returns nothing without one.
...
Casting can be controlled per account, so an administrator can share their Beacon without handing over the speakers: the policy is a whitelist or a blacklist, and a second switch decides what accounts not on the list - including ones that show up later - are allowed to do. Anyone who may not cast can still browse and listen locally. The policy is server-wide, kept per media server, the administrator is always allowed, and an untouched server lets everyone cast. It is offered in the Docker and web builds only - the desktop app runs a single-user backend of its own, where there is nobody to restrict. The picker offers the accounts that have signed in to this Beacon, plus Jellyfin's own full list. The account matched is the one the server reports for the signed-in credentials, not the name typed at login; on Plex, whose server cannot name its accounts, it is confirmed with plex.tv when someone signs in, so a Plex account that was already signed in before this Beacon version counts as not listed until it signs in again. See [docs/cast-permissions.md](docs/cast-permissions.md) for what this does and does not enforce.
...
**Mount a volume at `/data`** to keep AirPlay 2 pairings, your log-level setting, your account settings (language, personalized recommendations, lyrics providers, Autoplay batch size), the recommendations cache, the Fanart.tv image cache, API keys entered in Settings, each station's played-titles list, the casting allow-list, and (for Jellyfin/Plex sessions) your internet radio stations across container recreations/updates.
...
| `CONNECT_DATA_DIR`       | `/data` in Docker    | Directory persistent backend files are stored in - AirPlay 2 pairing credentials, the log-level setting, per-account settings, the recommendations cache, the Fanart.tv image cache, the API keys entered in Settings, resolved radio station logos, each account's radio title log, the casting allow-list, and (Jellyfin/Plex sessions only) internet radio stations. Docker already defaults this to `/data`; just mount a volume there.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `LASTFM_API_KEY`         | -                    | A Last.fm application key, which turns on the Last.fm playlist builder: it fills a new playlist from Last.fm (country or global charts, a genre's top tracks, one artist's top tracks, tracks similar to a song of your own, or your own listening history) using the tracks your library actually has. Usually there is no need to set this - the key can be entered in Settings under Advanced, API keys, instead, which is the only way on a desktop install, and it is stored in `CONNECT_DATA_DIR`. This variable is the equivalent for a Docker deployment that would rather configure it up front; a key entered in Settings takes precedence over it. Request a key at `last.fm/api/account/create`. The key belongs to the Beacon installation rather than to each listener: one covers everyone using it, who then only enter their own Last.fm username for the history-based playlist. With no key from either source, the builder is not offered. |
| `FANART_API_KEY`         | -                    | A personal Fanart.tv API key. Optional: Beacon ships its own project key, so the artist images work without it. A personal key is sent alongside it and is what Fanart.tv's terms ask each installation for; it can also be entered in Settings under Advanced, API keys, which takes precedence over this variable.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
**Persistent backend data** (AirPlay 2 pairing credentials, log-level setting, per-account settings, recommendations cache, Fanart.tv image cache, API keys, radio played-titles lists, Jellyfin/Plex internet radio stations) lives in Electron's standard per-user data directory, which survives app updates:
...
Features are a different matter, and it is only fair to say so up front: Beacon is deliberately opinionated. It is the player I want to use, and a good deal of what it deliberately does _not_ do is a decision rather than an oversight. So a feature PR may well get a friendly "thanks, but this one isn't for Beacon" - and if that happens it is almost never about the quality of the work, only about whether the feature belongs in this particular app.
