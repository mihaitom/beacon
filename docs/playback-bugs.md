...
the resume seeked FFmpeg past the track's own end), fixed separately below
under "Resuming an old interruption seeked past the track's own end". It
explains that follow-on symptom but says nothing about why the device
...
### Resuming an old interruption seeked past the track's own end (2026-08-24)
...
dropped in the first place - that's the still-open "Auto-advance onto a
still-playing device drops the next track silently" entry above (and the
larger still-open "Cast device drops a healthy stream mid-track" investigation
elsewhere in this file). Before this fix, an interruption's *recovery* was
